...
router.post("/register", async (req, res) => {
  ...
  if (!emailValue || !emailValue.includes("@") || !passwordValue) {
    return res
      .status(400)
      .json({ status: "false", message: "請輸入 email 或密碼" });
  }
  const isExistedUser = users.find((user) => user.email === emailValue);
  if (isExistedUser) {
    return res
      .status(400)
      .json({ status: "false", message: "此 email 已被註冊" });
  }
  const newUser = {
    id: nextId++,
    email: emailValue,
    password: await bcrypt.hash(passwordValue, 10),
  };
  users.push(newUser);
